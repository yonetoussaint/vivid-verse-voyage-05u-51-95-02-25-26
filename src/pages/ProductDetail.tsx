// Debug version of ProductDetail component
...
const ProductDetail = () => {
  ...
  // Wrap component in error boundary logic
  useEffect(() => {
  ...
  addDebug(`Component rendering with productId: ${productId}`);

  const { data: product, isLoading, error: productError } = useProduct(productId);
  const { data: analytics, isLoading: analyticsLoading, error: analyticsError } = useProductAnalytics(productId);

  useEffect(() => {
    addDebug(`Product loading state: ${isLoading}, Product error: ${productError?.message || 'none'}`);
    addDebug(`Product data: ${product ? 'loaded' : 'null'}`);
  }, [isLoading, productError, product]);

  useEffect(() => {
    addDebug(`Analytics loading state: ${analyticsLoading}, Analytics error: ${analyticsError?.message || 'none'}`);
  ...
  // Safely initialize useVariantStockDecay
  let variantStockInfo, activateVariant, getTimeRemaining, resetVariant, resetAllVariants;
  
  try {
    const stockDecayResult = useVariantStockDecay({
      variants: colorVariants,
      decayPeriod: 12 * 60 * 60 * 1000
    });
    
    variantStockInfo = stockDecayResult.variantStockInfo;
    activateVariant = stockDecayResult.activateVariant;
    getTimeRemaining = stockDecayResult.getTimeRemaining;
    resetVariant = stockDecayResult.resetVariant;
    resetAllVariants = stockDecayResult.resetAllVariants;
    
    addDebug('Variant stock decay initialized successfully');
  } catch (error) {
    addDebug(`Error initializing variant stock decay: ${error.message}`);
    // Fallback values
    variantStockInfo = {};
    activateVariant = () => {};
    getTimeRemaining = () => 0;
  ...
  useEffect(() => {
    if (selectedColor && activateVariant) {
      try {
        ...
        addDebug(`Activated variant: ${selectedColor}`);
      } catch (error) {
        addDebug(`Error activating variant ${selectedColor}: ${error.message}`);
      }
    }
  ...
  if (renderError) {
    return (
      <div className="flex flex-col items-center justify-center h-screen p-4">
        ...
        <div className="mt-8 p-4 bg-gray-100 rounded max-w-lg">
  ...
  // Show loading state
  if (isLoading) {
    addDebug('Showing loading state');
    return (
  ...
  // Show error state
  if (productError) {
    addDebug(`Showing error state: ${productError.message}`);
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <h1 className="text-2xl font-bold mb-4">Error Loading Product</h1>
        <p className="text-gray-500 mb-4">Error: {productError.message}</p>
  ...
  addDebug('Product loaded successfully, preparing to render main component');

  // Extract images and videos safely
  ...
  addDebug(`Images: ${images.length}, Videos: ${productVideos.length}`);
  ...
  addDebug('About to render main component');
  ...
  const triggerHaptic = async () => {
    try {
      ...
      addDebug(`Haptic error: ${error.message}`);
    }
  };
  ...
  const incrementQuantity = async () => {
    try {
      ...
      addDebug(`incrementQuantity error: ${error.message}`);
    }
  };
  ...
  const decrementQuantity = async () => {
    try {
      ...
      addDebug(`decrementQuantity error: ${error.message}`);
    }
  };
    ...
    try {
      ...
      if (navigator.share) {
        navigator.share({
          ...
          addDebug(`Share error: ${error.message}`);
        });
      } else {
      ...
      addDebug(`handleShare error: ${error.message}`);
    }
  };
  ...
  const toggleFavorite = async () => {
    try {
      ...
      addDebug(`toggleFavorite error: ${error.message}`);
    }
  };
  ...
  const addToCart = async () => {
    try {
      ...
      addDebug(`addToCart error: ${error.message}`);
    }
  };
  ...
  const buyNow = async () => {
    try {
      ...
      addDebug(`buyNow error: ${error.message}`);
    }
  };
  ...
  const handleCartClick = () => {
    try {
      ...
      addDebug(`handleCartClick error: ${error.message}`);
    }
  };
  ...
  const handleResetStock = () => {
    try {
      ...
      addDebug(`handleResetStock error: ${error.message}`);
    }
  };
  ...
  const scrollToSection = (section: string) => {
    try {
      ...
      addDebug(`scrollToSection error: ${error.message}`);
    }
  };
  ...
  useEffect(() => {
    const handleScroll = () => {
      try {
        ...
        addDebug(`handleScroll error: ${error.message}`);
      }
    };
  ...
  useEffect(() => {
    if (productVideos.length > 0) {
      addDebug(`Product videos found: ${productVideos.length}`);
  ...
  try {
    return (
      <div className="flex flex-col min-h-screen bg-white overscroll-none" ref={contentRef}>
        ...
        <div className="fixed top-0 right-0 z-50 p-2 bg-black/80 text-white text-xs max-w-xs max-h-32 overflow-y-auto">
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <h1 className="text-2xl font-bold mb-4 text-red-600">Render Error</h1>
